...
const sheetVariants = cva(
  `fixed  overflow-auto z-50 gap-4 bg-white p-6 shadow-lg transition ease-in-out 
  data-[state=open]:animate-in data-[state=closed]:animate-out 
  data-[state=closed]:duration-300 data-[state=open]:duration-500 dark:bg-default-dark`,
  {
    variants: {
      side: {
        ...
        left: "inset-y-0 left-0 h-full  w-full xs:w-3/4  data-[state=closed]:slide-out-to-left data-[state=open]:slide-in-from-left sm:max-w-lg",
        right:
...
const SheetHeader = ({
  ...
  <div
    className={cn("mt-5 flex flex-col space-y-4 text-left", className)}
    {...props}
  />
);
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Description
    ref={ref}
    className={cn(
      "pt-5 text-sm  text-light-main dark:text-dark-main",
      className,
    )}
    {...props}
  />
